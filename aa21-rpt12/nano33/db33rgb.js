...
parser.on('data', (data) => { // call back when data is received
    ...
    if (readData.lastIndexOf(',') > firstcommaidx && firstcommaidx > 0) {
        ...
        pres = readData.substring();
        rr = readData.substring();
        gg = readData.substring();
        bb = readData.substring();
        ...
        mdata[4]=;    // pressure data
        mdata[5]=;       // r_ratio
        mdata[6]=;       // g_ratio
        mdata[7]=bb;       // b_ratio
        //console.log(mdata);
        var iotData = new Sensor({date:dStr, temperature:temp, humidity:humi, luminosity:lux, pressure:, 
